...
3. **News** — `templates/news/{index,post}.html.twig` extend `base.html.twig` (the site shell), with scoped `.news-*` styles in `head_styles` on the site.css tokens (so they follow the light/dark toggle). The old self-contained `news/_layout.html.twig` "newsprint" theme was retired. `NewsController::render()` does not pass the social context, so news pages set `canonical_url` (and the post sets `og_title`/`og_image`) explicitly. The `news/` dir is excluded from OG auto-discovery in `generate-og.js` (per-post cards come from the news-manifest path).
