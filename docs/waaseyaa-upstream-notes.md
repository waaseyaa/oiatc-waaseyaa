...
## 021 — `waaseyaa/seo` has no per-page OG image generation

- **Date / version:** 2026-05-31 · waaseyaa/framework alpha.188
- **Doing:** Adding social cards to oiatc.ca. New explainer page `/explainers/where-your-data-lives` was sharing with the generic site OG card because there was no mechanism to produce a per-page one automatically.
- **Symptom:** `waaseyaa/seo` ships `SitemapGenerator`, `RobotsTxtGenerator`, `MetaTagBuilder`, `JsonLdBuilder`, and Twig helpers, but nothing that renders an OpenGraph image (e.g. `og:image` PNG) from page metadata. App authors have to either ship one default card for everything (uninformative previews) or hand-build a card per page (won't happen consistently for new pages).
- **Workaround:** Built it app-side as a `scripts/generate-og.js` Playwright-based generator that walks `templates/**/*.html.twig`, finds files extending `base.html.twig`, parses each page's `{% block title %}` and `{% block description %}`, and renders a card per page into `public/images/og/<slug>.png` at build time. Paired with a canonical override mechanism in `base.html.twig` (small per-line blocks: `og_image`, `og_title`, `og_description`, `canonical_url`, `robots`, `og_type`, `schema_jsonld`) so pages override one thing at a time without redeclaring the whole `head_meta` block, and a `HomeController::socialContext()` helper that maps template name → on-disk auto card → context vars consumed by base.
- **Likely upstream fix:** Add an OG image generator to `waaseyaa/seo` (or a sibling `waaseyaa/seo-og` package) that does the same walk/parse/render, exposes the canonical override blocks via a partial template (`@WaaseyaaSeo/head_meta.html.twig`) that app `base.html.twig` can include, and ships a `waaseyaa seo:og` CLI command for build/deploy hooks. Headless Chromium is the obvious renderer but the same shape works with any HTML-to-PNG backend (e.g. Browserless, server-side Satori). Keep it build-time so the framework doesn't pull a runtime browser dependency into production containers.
